...
- UI patterns: default to Headless UI components (Menu/Popover/Dialog/etc.) styled with Tailwind for interactive overlays, menus, and similar behaviors; reach for fully custom components only when no suitable Headless UI primitive exists or when Headless UI would not improve accessibility/UX. Make dismissal/positioning business rules configurable (e.g., whether item clicks, outside clicks, or scroll keep a menu open) and default those rules to closing when unspecified.
